...
export const wretchHomeworks = (id: number) => {
  return async (dispatch: Dispatch<HomeworkAction>) => {
    ...
    const homeworksRes = await baseWretch()
      .url(`api/Homeworks/by-group/${id}`)
      .get()
      .json();
...
export const wretchTabs = () => {
  return async (dispatch: Dispatch<HomeworkAction>) => {
    const tab1Res = await baseWretch().url(`api/groups/510`).get().json();
    const tab2Res = await baseWretch().url(`api/groups/2`).get().json();
